 ...
 * @param {Array<object>} options.allowedHosts - A Array of objects
 ...
 */
exports.validateRequest = ({allowedHosts, request}) => {
  const {ip, headers: {host}} = request;
  let verified = false;
  for(const entry of allowedHosts) {
    ...
    if(host === entry.host) {
      ...
      verified = entry.ips.includes(ip);
    }
  }
  return {verified};
};
