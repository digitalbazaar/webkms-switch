...
exports.runOperation = async ({operation, storage, moduleManager}) => {
  ...
  // `invocationTarget.controller` if op is generate key, otherwise it must
  // match existing key record `controller`
  ...
  await validator.verifyProof({operation, record});

  if(isGenerateKeyOp) {
    if(!record) {
      await storage.insert(operation.invocationTarget);
  ...
  if(isGenerateKeyOp) {
    ...
    await storage.update({id: keyId, record});
  }
