...
exports.validateOperation = async ({
  ...
  if(operation.type !== 'GenerateKeyOperation') {
    const invocationUrl = operation.invocationTarget.id || operation.invocationTarget
    // Parse the invocationTarget
    const {host: keyHost} = new URL(invocationUrl);
    if(keyHost !== expectedHost) {
      throw new Error(`kms expected ${expectedHost} in key id, but` +
        ` key's host is ${keyHost}`);
    }
    if(invocationUrl !== expectedTarget) {
      throw new Error(`expectedTarget is ${expectedTarget} but operation target is` +
        ` ${invocationUrl}`);
    }
  }
