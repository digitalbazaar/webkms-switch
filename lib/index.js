...
exports.validateOperation = async ({
  ...
  const targetUrl = new URL(operation.invocationTarget);
  //remove any subdomain from the host
  const {host} = targetUrl;
  if(host !== expectedHost) {
    throw new Error(
      `kms expected ${expectedHost} "invocationTarget"'s host is ${host}`);
  }
 ...
 * @param {Array<string>} options.allowedIps - A list of allowedIps
 *   for the request.
 * @param {string} options.expectedHost - The expected host of the request.
 * @param {object} options.request - An express request object.
 *
 * @returns {Promise<object>} An object with the verify result and any errors.
 */
exports.validateRequest = async ({allowedIps, expectedHost, request}) => {
  const {ip, headers} = request;
  if(!allowedIps.includes(ip)) {
    throw new Error(`${ip} not in kms list of allowedIps`);
  }
  const {host} = headers;
  if(host !== expectedHost) {
    throw new Error(`kms expected host ${expectedHost} got ${host}`);
  }
