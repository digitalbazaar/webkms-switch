...
export async function createMiddleware({
  ...
  return [
    ...
    asyncHandler(async function validateRequest(req, res, next) {
      await _validateRequest({req, storage});
      // get keystore config
      const {body: operation} = req;
      const keyId = operation.invocationTarget && operation.invocationTarget.id;
      const keystore = await storage.get({req, keyId});
...
async function _runOperation({req, moduleManager}) {
  // get WebKMS module
...
// eslint-disable-next-line no-unused-vars
async function _validateRequest({req, storage}) {
