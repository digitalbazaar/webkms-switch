...
exports.validateOperation = async ({
  ...
  const zcapInvocationResult = await verifyCapabilityInvocation({
    url, method, headers,
  ...
  const result = {
    valid: zcapInvocationResult.verified,
    ...zcapInvocationResult
  };

  if(result.valid) {
    // run key type validation if key is being generated
    const isGenerateKeyOp = operation.type === 'GenerateKeyOperation';
    if(isGenerateKeyOp) {
      const {valid, error} = await validateKeyType({operation});
      result.valid = valid;
      result.error = error;
    }
  }
...
exports.runOperation = async ({operation, storage, moduleManager}) => {
  ...
  if(isGenerateKeyOp) {
    ...
    // FIXME: this is a temporary measure until the kmsModule term moves
    // into the keystore config.
    // see: https://github.com/digitalbazaar/webkms-switch/issues/14
    // `record.key` already has the proper `@context` value which includes
    // the webkms context and it should not be overwritten by
    // `result['@context']` which only includes the suite context.
